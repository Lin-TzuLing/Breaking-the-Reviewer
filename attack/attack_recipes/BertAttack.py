...
from attack.adv_reviewer.search_method.greedy_word_swap_wir import (  # change output
    GreedyWordSwapWIR,
)
...
class BertAttack(AttackRecipe):
    ...
    def build(self):
        ...
        transformation = WordSwapMaskedLM(
            method="bert-attack",
            max_candidates=self.config["bertattack"]["max_candidates"],
        )
        ...
        constraints.append(
            MaxWordsPerturbed(
                max_percent=self.config["bertattack"]["max_word_perturbed_percent"]
            )
        )
        ...
        use_constraint = UniversalSentenceEncoder(
            threshold=self.config["bertattack"]["min_sentence_cos_sim"],
            metric="cosine",
        ...
        source = (
            "bertattack"
            if "query_budget" in self.config["bertattack"].keys()
            else "goal_function"
        )
        ...
        source = (
            "bertattack"
            if "top_k_wir" in self.config["bertattack"].keys()
            else "goal_function"
        )
        top_k_wir = self.config[source]["top_k_wir"]

        search_method = GreedyWordSwapWIR(
            wir_method="unk", query_budget=query_budget, top_k_wir=top_k_wir
        )
