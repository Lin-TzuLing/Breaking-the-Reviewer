...
def init_gpt2_model(checkpoint_dir, args, model_class, tokenizer_class=None):
    ...
    if tokenizer_class:
        tokenizer = tokenizer_class.from_pretrained(
            checkpoint_dir, do_lower_case=args.do_lower_case
        )
    else:
        tokenizer = None
...
class GPT2ParentModule(nn.Module):
    ...
    def generate(
        ...
        dense_length = (
            0 if style_content_vectors is None else len(style_content_vectors[0])
        )
...
def get_logits(model, iteration, generated, segments, style_content_vectors, past):
    if iteration == 0:
        if style_content_vectors is None:
            pred = model(input_ids=generated, token_type_ids=segments, return_dict=True)
        else:
            pred = model(
                input_ids=generated,
                token_type_ids=segments,
...
def get_logits_old(model, iteration, generated, segments, style_content_vectors, past):
    if iteration == 0:
        if style_content_vectors is None:
            logits, past = model(input_ids=generated, token_type_ids=segments)
        else:
            logits, past = model(
                input_ids=generated,
                token_type_ids=segments,
        ...
        # used the cached representations to speed up decoding
        logits, past = model(
            input_ids=generated[:, -1:], token_type_ids=segments[:, -1:], past=past
        )
    return logits, past
    ...
    with torch.no_grad():
        ...
        for i in range(new_length):
            ...
            if interpolation:
                ...
                final_probs = (
                    interpolation["weight"] * probs2
                    + (1 - interpolation["weight"]) * probs
                )
                next_token_logits = torch.log(final_probs) / (
...
def beam_search(
    ...
    with torch.no_grad():
                ...
                curr_element.append(
                    {
                        ...
                        "past": elem_num,
                        "sequence": [
                            x.unsqueeze(0).unsqueeze(0) for x in context[elem_num]
                        ]
        ...
        for i in range(1, new_length):
            ...
            all_eos = all(
                [beam["eos_emitted"] for element in all_beams for beam in element]
            )
            if all_eos:
                break
