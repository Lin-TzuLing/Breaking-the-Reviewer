    ...
    def generate_batch(
        ...
        output, _, scores = self.gpt2_model.generate(
            gpt2_sentences=torch.tensor(
                np.array([inst.sentence for inst in instances])
            ).to(args.device),
            segments=torch.tensor(np.array([inst.segment for inst in instances])).to(
                args.device
            ),
            global_dense_vectors=torch.tensor(
                np.array([inst.gdv for inst in instances])
            ).to(args.device),
