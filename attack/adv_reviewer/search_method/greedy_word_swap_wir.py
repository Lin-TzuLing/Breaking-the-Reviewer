...
class GreedyWordSwapWIR(SearchMethod):
    ...
    Args:
        ...
        query_budget: budget for the number of queries
        top_k_wir: number of top words to consider for WIR
    """

    def __init__(
        self, query_budget, top_k_wir=None, wir_method="unk", unk_token="[UNK]"
    ):
        ...
        self.top_k_wir = top_k_wir
        self.first_output_as_current = None
    ...
    def perform_search(self, initial_result):
        ...
        # Only consider the top k words with the highest importance
        if self.top_k_wir is not None:
            index_order = index_order[: self.top_k_wir]
            print(
                f"Only consider the top {self.top_k_wir} words with the highest importance"
            )

        transformed_text_candidates = []
        while i < len(index_order) and not search_over:

            transformed_text_candidates.extend(
        ...
        if len(transformed_text_candidates) == 0:
            results, search_over = [cur_result], True
        else:
        ...
        # self-added code
        # Choose the result with the highest score, even if the score shift is negative.
        # Assign the first output with transformation as current result
        if self.first_output_as_current is None:
            cur_result = results[0]
            self.first_output_as_current = "set first result as current result"
